...
**Relative Paths**
:   Start from your current directory. Shorter but depend on where you are. If a path does not start with `/` or `~`, Unix assumes it is relative to your current working directory.
```bash
Documents/analysis.R
```
...
When you first open a terminal, it typically starts in your home directory. The output of `pwd` will vary depending on your system—you might see `/Users/yourusername` on macOS, `/home/yourusername` on Linux, or `/c/Users/yourusername` on Git Bash for Windows.
...
Your shell can be customized through configuration files that run when you open a terminal:
...
Several other commands are worth knowing about as you advance:
...
| `awk` | Powerful text processing (covered in @sec-grep-regex) |
